...
const Card = ({filter_name, addorder,table_number }) => {
	...
	const Handler = async(id,name,prize,url)=>
	{
		if(table_number !== null){
			...
			await resetTableNumber           //once we select the filter and order next time reset the tablenumber and filter
			await resetFilter
			...
			<center>
				...
				{data.length > 0 ?
					<div className='container'>
						<div className='row'>
							{data.map((item) => (                    //Hydrate the data in rows and then print in col
								<div className='col-md-4' key={item.id} >
									<div className='card mb-4'>
										<img src={item.url} className='card-image-top' alt='image not found' />
